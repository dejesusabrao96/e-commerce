document.addEventListener("DOMContentLoaded", () => {
  new ApexCharts(document.querySelector("#reportrantinan"), {
    series: [{
      name: 'A⁺',
      data: ["{% for xx in loopingetinanRAnTama %}","{{xx.total1}}","{% endfor %}"]
    },{
      name: 'Aˉ',
      data: ["{% for xx in loopingetinanRAnTama %}","{{xx.total3|floatformat:'3'}}","{% endfor %}"]
    },{
      name: 'B⁺',
      data: ["{% for xx in loopingetinanRAnTama %}","{{xx.total2|floatformat:'3'}}","{% endfor %}"]
    },{
      name: 'Bˉ',
      data: ["{% for xx in loopingetinanRAnTama %}","{{xx.total4|floatformat:'3'}}","{% endfor %}"]
    // },{
    //   name: 'AB⁺',
